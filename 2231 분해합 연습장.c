...
자릿수마다 분해합 함수를 54번 돌리고 범위를 십의자리부터 십만자리까지
살펴보고 입력받은 값과 분해합의 값을 비교하면 출력하는 형식을 선택함. 
이렇게 되면 모두 살펴보는 과정에서 의도치 않은 한개 이상의 출력이 
나올 수 있다는 점을 간과함. 그래서 수정을 해보자면 예를들어 216을 입력
한다면 215번의 함수를 돌려서 분해합을 찾는다. 이때 구해진 분해합은 가장
작은 분해합이기 때문에 답을 구했다. 만약 이때까지 분해합이 구해지지 않았
다면 0을 출력한다. 추가적으로위의 변수 네이밍을 보면 알겠지만 매우
지저분함. 의도를 알수없는 변수 네이밍때문에 헷갈리게 되고, 이는
재정립할 필요가 있어보임.
